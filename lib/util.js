...
export const util = {
  ...
  parseCommand: function (command, commands) {
    ...
    function parsePipes() {
      ...
      for (const key in naivePipes) {
        ...
        for (let i = 0, l = possiblePipe.length; i < l; i++) {
    ...
    if (!match) {
      ...
      if (match) {
        ...
        for (const key in allCommands) {
          ...
          let matchAll = true;
          for (let k = 0, kl = cmdParts.length; k < kl; k++) {
  ...
  buildCommandArgs: function (passedArgs, cmd, execCommand, isCommandArgKeyPairNormalized) {
    ...
    types.boolean = booleans.map(function (str) {
      ...
      for (let i = 0, l = passedArgParts.length; i < l; i++) {
    ...
    for (let m = 0, ml = cmd.options.length; m < ml; m++) {
  ...
  prettifyArray: function (arr = []) {
    const arrClone = _.clone(arr),
          ...
          longest = strip((arrClone.sort((a, b) => {
            return strip(b).length - strip(a).length;
          })[0] || '')).length + 2,
          fullWidth = strip(String(arr.join(''))).length,
          fitsOneLine = ((fullWidth + (arr.length * 2)) <= width);
